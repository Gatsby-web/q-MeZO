...
ALL_ARGS_TOGETHER="
    ...
    --overwrite_output_dir --output_dir result_adam/$TASK-$MODEL-$TYPE-$TRAINER-$TAG$GRID_TAG/$K-$SEED
    --num_k $K
    --tag $TAG
    --max_seq_length 128
    --seed $SEED
...
if [[ $NUM_GPU > 1 ]]; then
    ...
    python -m torch.distributed.launch --nproc_per_node $NUM_GPU --master_port $PORT_ID run_adam.py \
        $ALL_ARGS_TOGETHER
else
    python run_adam.py \
        $ALL_ARGS_TOGETHER
